...
static inline int ieee802154_cc13xx_cc26xx_subg_channel_to_frequency(uint16_t channel, uint16_t *frequency, uint16_t *fractFreq)
{
	...
	if (channel == 0) {
		*frequency = 868;
		 ...
		 */
		*fractFreq = 0x4ccd;
	} else if (1 <= channel && channel <= 10) {
		*frequency = 906 + 2 * (channel - 1);
		*fractFreq = 0;
	} else if (11 <= channel && channel <= 26) {
...
static void cmd_prop_tx_adv_callback(RF_Handle h, RF_CmdHandle ch, RF_EventMask e)
{
	const struct device *dev = &DEVICE_NAME_GET(ieee802154_cc13xx_cc26xx_subg);
...
static void cmd_prop_rx_adv_callback(RF_Handle h, RF_CmdHandle ch, RF_EventMask e)
{
	const struct device *dev = &DEVICE_NAME_GET(ieee802154_cc13xx_cc26xx_subg);
	struct ieee802154_cc13xx_cc26xx_subg_data *drv_data = get_dev_data(dev);
	RF_Op *op = RF_getCmdOp(h, ch);

	LOG_DBG("ch: %u cmd: %04x st: %04x e: 0x%" PRIx64, ch, op->commandNo, op->status, e);

	if (e & RF_EventRxEntryDone) {
		ieee802154_cc13xx_cc26xx_subg_rx_done(drv_data);
	}

	if (op->status == PROP_ERROR_RXBUF || op->status == PROP_ERROR_RXFULL || op->status == PROP_ERROR_RXOVF) {
...
static int ieee802154_cc13xx_cc26xx_subg_set_channel(const struct device *dev,
						     uint16_t channel)
{
	struct ieee802154_cc13xx_cc26xx_subg_data *drv_data = get_dev_data(dev);
	RF_EventMask reason;
	...
	r = ieee802154_cc13xx_cc26xx_subg_channel_to_frequency(channel, &freq, &fract);
	if (r < 0) {
		return -EINVAL;
	}
...
static int ieee802154_cc13xx_cc26xx_subg_set_txpower(const struct device *dev,
						     int16_t dbm)
{
	struct ieee802154_cc13xx_cc26xx_subg_data *drv_data = get_dev_data(dev);
	RF_Stat status;
					    ...
					    struct net_buf *frag)
{
	...
	memcpy(&drv_data->tx_data[IEEE802154_SUN_PHY_FSK_PHR_LEN], frag->data, frag->len);

	/* Chain commands */
	drv_data->cmd_prop_cs.pNextOp = (rfc_radioOp_t *) &drv_data->cmd_prop_tx_adv;
	drv_data->cmd_prop_cs.condition.rule = COND_STOP_ON_TRUE;

	/* Set TX data */
	drv_data->cmd_prop_tx_adv.pktLen = frag->len + IEEE802154_SUN_PHY_FSK_PHR_LEN;
	...
	do {
		...
		if (drv_data->cmd_prop_cs.status != PROP_DONE_IDLE) {
			LOG_DBG("Channel access failure (0x%x)", drv_data->cmd_prop_cs.status);
			/* Collision Avoidance is a WIP
			 * Currently, we just wait a random amount of us in the range [0,256)
			 * but k_busy_wait() is fairly inaccurate in practice. Future revisions
			 * may attempt to use the RAdio Timer (RAT) to measure this somewhat
			 * more precisely.
			 */
...
static inline uint8_t ieee802154_cc13xx_cc26xx_subg_convert_rssi(int8_t rssi)
{
...
uint16_t ieee802154_cc13xx_cc26xx_subg_get_subg_channel_count(const struct device *dev)
{
	ARG_UNUSED(dev);
...
static void ieee802154_cc13xx_cc26xx_subg_setup_rx_buffers(struct ieee802154_cc13xx_cc26xx_subg_data *drv_data)
{
	for (size_t i = 0; i < CC13XX_CC26XX_NUM_RX_BUF; ++i) {
		...
		if (i < CC13XX_CC26XX_NUM_RX_BUF - 1) {
			drv_data->rx_entry[i].pNextEntry = (uint8_t *) &drv_data->rx_entry[i + 1];
		} else {
			drv_data->rx_entry[i].pNextEntry = (uint8_t *) &drv_data->rx_entry[0];
		}
...
static void ieee802154_cc13xx_cc26xx_subg_data_init(struct ieee802154_cc13xx_cc26xx_subg_data *drv_data)
{
	uint8_t *mac;
...
static struct ieee802154_radio_api ieee802154_cc13xx_cc26xx_subg_radio_api = {
	...
	.get_subg_channel_count = ieee802154_cc13xx_cc26xx_subg_get_subg_channel_count,
};
...
static struct ieee802154_cc13xx_cc26xx_subg_data ieee802154_cc13xx_cc26xx_subg_data = {
	...
	.cmd_prop_rx_adv = {
		...
		.pQueue = &ieee802154_cc13xx_cc26xx_subg_data.rx_queue,
		.pOutput = (uint8_t *) &ieee802154_cc13xx_cc26xx_subg_data.cmd_prop_rx_adv_output,
	},
