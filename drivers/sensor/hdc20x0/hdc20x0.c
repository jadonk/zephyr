...
#define hdc20x0_REG_MEASUREMENT_CONF 0x0f
...
static int
hdc20x0_read_measurement_temperature(struct hdc20x0_data *data,
...
static int hdc20x0_read_measurement_humidity(struct hdc20x0_data *data,
					     const struct hdc20x0_cfg *config)
{
	uint8_t val;
	float rh;
...
/*measurement humidity and temperature*/
static int hdc20x0_read_measurement_all(struct hdc20x0_data *data,
					const struct hdc20x0_cfg *config)
{
	if (hdc20x0_read_measurement_temperature(data, config))
		return -EIO;
	if (hdc20x0_read_measurement_humidity(data, config))
		return -EIO;
	return 0;
}
				...
				enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_ALL:
		if (hdc20x0_read_measurement_all(data, config)) {
