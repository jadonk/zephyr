...
static int greybus_i2c_control_init(struct device *dev) {
	...
	struct greybus_i2c_control_data *drv_data =
        (struct greybus_i2c_control_data *)dev->driver_data;
    struct greybus_i2c_control_config *config =
        (struct greybus_i2c_control_config *)dev->config_info;
    int r;
    struct device *bus;
    ...
    if (NULL == api) {
		LOG_ERR("i2c control: failed to get driver_api for '%s'", config->bus_name);
    	return -EINVAL;
    }
