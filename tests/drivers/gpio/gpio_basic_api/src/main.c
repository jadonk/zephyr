				      ...
				      gpio_port_pins_t pins)
{
	...
	r = gpio_sim_flags_get(port, PIN_OUT, &output_flags);
	__ASSERT(r == 0, "gpio_sim_flags_get() failed: %d", r);
	r = gpio_sim_flags_get(port, PIN_IN, &input_flags);
	__ASSERT(r == 0, "gpio_sim_flags_get() failed: %d", r);

	if ((output_flags & GPIO_OUTPUT) && (input_flags & GPIO_INPUT)) {
		r = gpio_sim_output_get(port, PIN_OUT);
		__ASSERT(r == 0 || r == 1, "gpio_sim_output_get() failed: %d", r);
		val = r;
		r = gpio_sim_input_set(port, PIN_IN, val);
